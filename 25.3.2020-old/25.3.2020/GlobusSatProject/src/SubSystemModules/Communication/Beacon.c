...
int BeaconSetBitrate()
{
	...
	/*if (g_current_beacon_period % g_beacon_change_baud_period == 0) {
       ...
       //g_current_beacon_period++; //םופר כמה פעמים עשה שידור
	return err;
}

void BeaconLogic()
{
	printf("Im in the beacon function\n");
	...
	{
		printf("time returned false\n");
		return;
	}

	printf( "Hello entering to collect beacon information\n" );
