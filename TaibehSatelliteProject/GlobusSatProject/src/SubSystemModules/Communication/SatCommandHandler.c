...
CommandHandlerErr AssembleCommand(unsigned char *data, unsigned short data_length, char type,char subtype, unsigned int id, sat_packet_t *cmd)
{
	if (NULL == cmd) {
		return cmd_null_pointer_error;
	}
	...
	if (NULL != data) {
		unsigned short size = (data_length > MAX_COMMAND_DATA_LENGTH) ?
