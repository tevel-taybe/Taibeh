...
int CMD_GetSatTime(sat_packet_t *cmd)
{
	...
	TransmitDataAsSPL_Packet(cmd, (unsigned char*) &curr_time,
			sizeof(curr_time));
...
int CMD_ResetComponent(reset_type_t rst_type)
{
	...
	case reset_eps:
		...
		FRAM_write(&reset_flag, RESET_CMD_FLAG_ADDR, RESET_CMD_FLAG_SIZE);
		vTaskDelay(10); // vTaskYIELD();
#ifdef ISISEPS
		//ieps_statcmd_t ieps_cmd;
		isis_eps__reset__to_t cmd;
		cmd.fields.rst_key = RESET_KEY;
		isis_eps__reset__from_t response;
		err = isis_eps__reset__tmtc( EPS_I2C_BUS_INDEX, &cmd, &response);
