#include <string.h>
#include <hal/errors.h>
#include <stdio.h>
#include "TRXVU.h"
#include "ActUponCommand.h"
#include "CommandDictionary.h"
#include "AckHandler.h"

int ActUponCommand(sat_packet_t *cmd)
{
	int err = 0;
	if (NULL == cmd){
		return E_NOT_INITIALIZED;
	}

	switch ((spl_command_type) cmd->cmd_type)
	{
	case trxvu_cmd_type: /// hex code 0x00
		printf(" +++++++trxvue cmd type === comamnd is %d %d  +++++++\n", (int)cmd->cmd_type, (int)cmd->cmd_subtype);
		err = trxvu_command_router(cmd);
		break;
	case eps_cmd_type: /// hex code 0x01
		err = eps_command_router(cmd);
		break;
	case telemetry_cmd_type: /// hex code 0x02
		err = telemetry_command_router(cmd);
		break;
	case filesystem_cmd_type: /// hex code 0x03
		err = filesystem_command_router(cmd);
		break;
	case managment_cmd_type: /// hex code 0x04
		err = managment_command_router(cmd);
		break;
	case maintenace_cmd_type: /// hex code 0x05
		err = maintenance_command_router(cmd);
		break;

	case ack_type: 		/// hex code 0x06						//this command is a ping function
		SendAckPacket(ACK_PING, cmd,NULL,0);
		break;
	}
	return err;
}


